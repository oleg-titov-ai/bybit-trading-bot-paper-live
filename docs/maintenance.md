...
> Current maintenance focus (2026-09-16): bind paper-mode readiness to one monotonic validation generation covering synthetic order intent, flags, position mode, risk settings, connection state, and fresh instrument metadata; cancel or ignore in-flight results whenever any trading-relevant input changes so stale async validation can never revive readiness.
